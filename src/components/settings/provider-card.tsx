        ...
        <CardHeader className="p-6 border-b border-border/50 bg-background/50">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full sm:w-auto mt-2 sm:mt-0">
              <Badge className={`px-2.5 py-1 text-xs font-medium tracking-wide shadow-none border ${getStatusColor(status)} flex items-center gap-1.5`}>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <Input
                    ...
                    onChange={(e) => onUpdate(provider.id, { apiKey: e.target.value, dirtyApiKey: true })}
                    onBlur={() => {
                      if (provider.baseUrl && provider.apiKey) onAutoTestAndSave({ ...provider, apiKey: provider.apiKey })
                    }}
          ...
          {(provider.id !== 'docscan-free-builtin' || provider.models.length > 1) && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="space-y-2">
                ...
                <Select value={provider.model} onValueChange={(value) => onUpdate(provider.id, { model: value })}>
              ...
              {!provider.id.startsWith('docscan-free') && (<>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => onUpdate(provider.id, { maxTokens: parseInt(e.target.value) })}
                    className="bg-background shadow-sm rounded-xl focus-visible:ring-1 focus-visible:ring-primary"
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => onUpdate(provider.id, { temperature: parseFloat(e.target.value) })}
                    className="bg-background shadow-sm rounded-xl focus-visible:ring-1 focus-visible:ring-primary"
                  />
                </div>
                <div className="space-y-2 md:col-span-3">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      onUpdate(provider.id, { costPer1kTokens: next === '' ? undefined : parseFloat(next) })
                    }}
