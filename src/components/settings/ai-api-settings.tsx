...
import { Settings, CheckCircle, Loader2, Save, Brain, Activity } from 'lucide-react'
...
export function AiApiSettings() {
  ...
  const [responsePreferences, setResponsePreferences] = useState<UserResponsePreferences>(
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        let mapped: AIProvider[] = (settingsData as any[]).map((s: any, i: number) =>
          mapServerDataToProvider(s, i)
        )
        ...
        if (freeConfig) {
          ...
          for (const config of [...configs].reverse()) {
            const existingIndex = mapped.findIndex(
              (m) => m.baseUrl === config.baseUrl && m.type === config.type
            )
            if (existingIndex >= 0) {
        ...
        Promise.all(
          mapped.map(async (p) => {
            if (
              !p.apiKey &&
              !['ollama', 'lm-studio'].includes(p.type) &&
              !p.id.startsWith('docscan-free')
            )
              return
            try {
              const { authenticatedRequest: ar } = await import('@/lib/api-client')
              setProviders((prev) =>
                prev.map((pp) => (pp.id === p.id ? { ...pp, testStatus: 'pending' } : pp))
              )
              const [testResult, modelsResult] = await Promise.allSettled([
                ar('/api/test-connection', {
                  method: 'POST',
              ...
              const newModels =
                modelsResult.status === 'fulfilled' && modelsResult.value?.models?.length > 0
        ...
        toast({
          ...
          description: 'Using default provider configurations.',
          variant: 'destructive',
        })
      }
    }
  ...
  const saveProviders = async (newProviders: AIProvider[]) => {
    ...
    const filteredToSave = newProviders.filter(
      (p) => p.isConfigured || p.dirtyApiKey || ['ollama', 'lm-studio'].includes(p.type)
    )
  ...
  const saveResponseControls = async () => {
    ...
    try {
      const { authenticatedRequest } = await import('@/lib/api-client')
      const saved = await authenticatedRequest<UserResponsePreferences>(
      ...
      toast({
        title: 'Response controls saved',
        description: 'AI response behavior settings were updated.',
      })
    } catch {
      toast({
        title: 'Failed to save response controls',
        description: 'Please try again.',
        variant: 'destructive',
      })
    } finally {
  ...
  const autoTestAndSave = async (provider: AIProvider) => {
    if (
      !provider.apiKey &&
      !['ollama', 'lm-studio'].includes(provider.type) &&
    ...
    try {
      ...
      if (ok) {
        ...
        const errorMsg =
          testResult.status === 'fulfilled' ? testResult.value.error : 'Connection failed'
      ...
      <Card className="shadow-sm border-border bg-card overflow-hidden">
        <CardHeader className="p-6 md:p-8 bg-background relative z-10">
          ...
          <CardDescription className="text-sm mt-1">
            Configure and manage your AI service providers. Only one provider can be active at a
            time.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6 md:p-8 relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-t border-border/50 bg-secondary/10">
          <div className="flex flex-wrap items-center gap-3">
            <Badge
              variant="outline"
              className="px-3 py-1.5 text-sm font-medium bg-background shadow-sm gap-1.5 font-normal"
            >
            ...
            <Badge
              variant="outline"
              className="px-3 py-1.5 text-sm font-medium bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800/50 text-emerald-700 dark:text-emerald-400 gap-1.5 shadow-sm font-normal"
            >
      ...
      <Tabs defaultValue="providers" className="space-y-6">
        <div className="bg-background rounded-2xl p-1.5 shadow-sm border border-border inline-flex w-fit">
          <TabsList className="bg-transparent h-auto p-0 flex gap-1">
            <TabsTrigger
              value="providers"
              className="rounded-xl data-[state=active]:bg-card data-[state=active]:text-foreground font-medium py-2 px-6 transition-all shadow-none text-muted-foreground border border-transparent data-[state=active]:border-border data-[state=active]:shadow-sm"
            >
              Providers
            </TabsTrigger>
            <TabsTrigger
        ...
        <TabsContent value="providers" className="space-y-6 m-0 outline-none">
          ...
          <Card className="shadow-sm border-border bg-card">
            ...
            <CardContent className="p-6 space-y-5">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="flex items-center justify-between rounded-xl border border-border/50 p-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Include source snippets in responses.
                    </p>
                  </div>
                  <Switch
                    checked={responsePreferences.reference_enabled}
                    onCheckedChange={(checked) =>
                      setResponsePreferences((prev) => ({ ...prev, reference_enabled: checked }))
                    }
                  />
                </div>
                <div className="flex items-center justify-between rounded-xl border border-border/50 p-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Return important text blocks in answers.
                    </p>
                  </div>
                  <Switch
                    checked={responsePreferences.highlight_enabled}
                    onCheckedChange={(checked) =>
                      setResponsePreferences((prev) => ({ ...prev, highlight_enabled: checked }))
                    }
                  />
                </div>
                <div className="flex items-center justify-between rounded-xl border border-border/50 p-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Use feedback history to adapt future responses.
                    </p>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setResponsePreferences((prev) => ({
                        ...prev,
                        memory_learning_enabled: checked,
                      }))
                    }
                  />
                </div>
                <div className="flex items-center justify-between rounded-xl border border-border/50 p-4">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Regenerate an improved response after feedback.
                    </p>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setResponsePreferences((prev) => ({
                        ...prev,
                        auto_regenerate_on_dislike: checked,
                      }))
                    }
                  />
                </div>
                <div className="flex items-center justify-between rounded-xl border border-border/50 p-4 md:col-span-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Show Ask with DocMind button on selected text in preview.
                    </p>
                  </div>
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setResponsePreferences((prev) => ({
                        ...prev,
                        preview_selection_enabled: checked,
                      }))
                    }
                  />
                </div>
              ...
              <Button
                onClick={saveResponseControls}
                disabled={savingResponsePreferences}
                className="w-full sm:w-auto"
              >
                {savingResponsePreferences ? (
                  <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                ) : (
                  <Save className="w-4 h-4 mr-2" />
                )}
