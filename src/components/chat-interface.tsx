...
import { Sparkles, ChevronDown, ChevronUp, Paperclip, Trash2 } from 'lucide-react'
...
export function ChatInterface({
  ...
  useEffect(() => {
    const loadPreferences = async () => {
      try {
        ...
        const prefs = await authenticatedRequest<{ auto_regenerate_on_dislike?: boolean }>(
  ...
  const addStreamDebugEntry = useCallback(
    (message: string) => {
      if (!streamDebugEnabled) return
      setStreamDebugEntries((prev) => [
        ...prev.slice(-23),
        {
  ...
  const handleFeedback = async (id: string, type: 'up' | 'down') => {
    ...
    if (type === 'down') {
      const prompted = window.prompt(
        'Tell DocMind what was wrong so it can improve the next response:',
        ''
      )
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (typeof ej?.error === 'string') errorMessage = ej.error
        } catch {}
  ...
  useEffect(() => {
    ...
    if (initialDocumentId) {
      setSelectedDocIds((prev) =>
        prev.includes(initialDocumentId) ? prev : [...prev, initialDocumentId]
      )
    }
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        ...
        authenticatedFetch(processorEndpoint, {
          ...
          .then(async (r) => {
            if (!r.ok) throw new Error((await r.text()) || `Processing failed for ${result.name}`)
          })
  ...
  const hasActiveStreamingMessage =
    !!activeStreamingMessageId && messages.some((msg) => msg.id === activeStreamingMessageId)
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div
        className="flex-1 min-h-0 overflow-y-auto px-1 scroll-smooth"
          ...
          <div className="space-y-5 py-4">
            {messages.map((msg) => (
              <MessageBubble
                ...
                copied={copied}
                showStreamingCursor={
                  msg.id === activeStreamingMessageId && hasActiveStreamingMessage
                }
              />
            ))}
            <AnimatePresence>
